    ...
    var graph = {
      ...
      attachEvent () {
        ...
        $('.list').on('click', '.zan>span', function (e) {
          ...
          graph.methods.agreeMessage($(this).parents('li'), type)
        })
        // 点赞和投诉文章库
        $('.statistics').on('click', '.zan,.complaint', function (e) {
          if ($(this).hasClass('complaint')) { // 投诉操作
            ...
            $(this).removeClass('agree')
            graph.methods.agreeArticle($(this).parents('.statistics'), 'remove')
          } else { // 点赞
            $(this).addClass('agree')
            graph.methods.agreeArticle($(this).parents('.statistics'), 'add')
          }
        })
        // 留言操作
        $('.message>a').click(function (e) {
          graph.methods.callMessage()
        })
        // 关闭和提交留言
        $('.modal-message').on('click', '.mask,.submit', function (e) {
          if ($(this).hasClass('submit')) { // 提交留言
            ...
            if (message) {
              graph.methods.sendMessage(message)
            }
          }
          $('.modal-message').hide()
            .find('.mask').fadeOut()
